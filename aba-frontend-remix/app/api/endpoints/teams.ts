...
import { ITeam, ITeamPlayer } from "~/lib/teams/types";
...
export const TeamsApi = (instance: AxiosInstance) => ({
  ...
  async getTeamsByUserId(userId?: string) {
    try {
      const { data: teamsData } = await instance.get<ITeamPlayer[]>(
        `runners/${userId}/teams`
      );
  ...
  // async getRegistrations({ page, limit }: { page: string; limit: string }) {
  //   try {
  ...
  async getPersonalCalendar(runnerId: number) {
    try {
      const { data: calendar } = await instance.get<IRegistration[]>(
        `/event-race-registrations/user/${runnerId}`
      );
      return calendar;
    } catch (error) {
      console.log(error);
