            ...
            >
              ...
              <div className="mb-4">
                <h3 className="text-xl uppercase font-semibold">
                  {match.eventRaceType?.event.title}
                </h3>
                <div className="flex gap-2 items-center">
                  <img
                    src={getCountryFlagSrc(
                      match.eventRaceType?.event.location?.country?.shortName
                    )}
                    alt="flag"
                    width={24}
                  />
                  <p className="text-gray-400 text-sm">
                    {match.eventRaceType?.event.location?.city},{" "}
                    {match.eventRaceType?.event.location?.country.name}
                  </p>
                </div>
              </div>
              <div className="mb-2">
                <h3 className="text-xl font-semibold mb-2">Teams:</h3>
                <h3 className="uppercase max-w-[250px]">{match.name}</h3>
              </div>
