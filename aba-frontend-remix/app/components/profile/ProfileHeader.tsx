...
import { IRunner, IUser } from "~/lib/types";
import {
  formatDate,
  getCategoryByDoB,
  getCountryFlagSrc,
...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ user, authedUser }) => {
  ...
  const onAddProfileImage = async (dto: any) => {
    if (!dto || !user) return;
    try {
      await Api(authedUser?.token).users.updateImage(dto.id);
            ...
            <div className="relative w-full md:w-auto flex justify-center mt-4 md:mt-[7%]">
              ...
              <div className="absolute -top-6 px-5 py-1 bg-red-700 rounded-md left-1/2 -translate-x-1/2">
                <p className="text-white text-xl font-semibold">
                  {
                    user.roles?.find((ur) => ur.role.stripe_product_id)?.role
                      .name
                  }
                </p>
              </div>
              ...
              <div className="flex-1 md:mt-6">
                ...
                <div className="flex gap-2 items-center mb-5">
                  {user.country.shortName && (
                    <img
                      src={getCountryFlagSrc(user.country.shortName)}
                      alt="flag"
                      width={40}
                    />
                  )}
                ...
                {isRunner(user) && (
                  <p className="font-semibold text-xl">
                    {formatDate((user as IRunner).dateOfBirth, false)} |{" "}
                    {getCategoryByDoB((user as IRunner).dateOfBirth)}
                  </p>
                )}
              </div>
              {isRunner(user) && (
                <div className="w-auto flex flex-col items-center justify-end">
                  <div className="rounded-full w-16 h-16 md:w-24 md:h-24 border-green-400 border-4 bg-green-400/50 flex justify-center items-center mb-4">
                    <h3 className="font-semibold text-2xl md:text-3xl">
                      {(user as IRunner).rank && (user as any).rank < 9000
                        ? (user as IRunner).rank
                        : "-"}
                    </h3>
                  </div>
                  <p className="font-semibold text-xl">
                    {(user as IRunner).gender.name === "male"
                      ? "Men's ranking"
                      : "Women's ranking"}
                  </p>
                </div>
