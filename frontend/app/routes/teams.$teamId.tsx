...
import { IRace, TeamResult } from "~/lib/types";
...
const TeamPage = () => {
  ...
  const [pbs, setPbs] = useState<TeamResult[]>([]);
  ...
  useEffect(() => {
    if (!teamResultsData?.results || !teamResultsData.results.length) return;
    const pb = teamResultsData.results.reduce(
      (acc, curr) =>
        acc.teamResult_resultInMs < curr.teamResult_resultInMs ? acc : curr,
      teamResultsData.results[0]
    );
    setPbs([pb]);
  }, []);
              ...
              <div className="flex-1 md:mt-6">
                ...
                <div className="flex gap-2 items-center mb-5">
                  {team.country.flagIconUrl && (
                    <img src={team.country.flagIconUrl} alt="flag" width={40} />
                  )}
