...
const CustomDrawer:  React.FC<CustomDrawer> = ({ open, setOpen }) => {
    ...
    return (
        <Drawer open={open} onClose={() => setOpen(false)}>
            <div className={'flex flex-col items-center h-screen bg-[#1E1C1F] p-2'}  onClick={() => setOpen(false)}
                ...
                {session?.user ? (
                    <div className={'py-5 px-3 flex justify-center w-full relative'}>
                        ...
                        <div className={'flex flex-col justify-between mr-3'}>
                            ...
                            <p className={'text-lg text-white'}>{session.user.name} {session.user.surname}</p>
                        </div>
                ...
                <div className={'w-full p-3'}>
                    <Link href="/">
                        <p className={`hover:opacity-80 py-2 border-b-2 border-solid border-red-300/10 text-xl uppercase ${pathname === '/' ? 'text-[#FF0000]' : 'text-white'}`}>Home</p>
                    </Link>
                    <Link href="/calendar">
                        <p className={`hover:opacity-80 py-2 border-b-2 border-solid border-red-300/10 text-xl uppercase ${pathname === '/calendar' ? 'text-[#FF0000]' : 'text-white'}`}>Calendar</p>
                    </Link>
                    <Link href="/close-events">
                        <p className={`hover:opacity-80 py-2 border-b-2 border-solid border-red-300/10 text-xl uppercase ${pathname === '/close-events' ? 'text-[#FF0000]' : 'text-white'}`}>Close Events</p>
                    </Link>
                    {session?.user && (
                        <Link className="hover:opacity-80" href="/register-team">
                            <p className={`text-xl uppercase py-2 ${pathname === '/register-team' ? 'text-[#FF0000]' : 'text-white'}`}>Register Team</p>
                        </Link>
                    )}
                </div>
            </div>
